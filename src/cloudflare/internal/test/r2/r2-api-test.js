...
async function compareResponse(res, { head, body } = {}) {
  ...
  const { body: _body, ...obj } = awaited;
  const { toJSON, ...checksums } = obj.checksums;
  ...
  if (body) {
    assert.strictEqual(await awaited.text(), body);
  }
}
  ...
  async test(ctrl, env, ctx) {
    // Basic Operations
    {
      ...
      // ListObjects
      {
        ...
        const { toJSON, ...checksums } = list.objects[0].checksums;
        list.objects[0].checksums = checksums;
